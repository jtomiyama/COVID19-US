...
if (file.exists(resultsFileName)){
  ...
  ### Make predictions out to max date
  maxIdx <- min(nrow(stateDataFiltered), minIdx + 120)
  epidemicStartDate <- stateDataFiltered$date[minIdx]
  epidemicEndDate <- stateDataFiltered$date[maxIdx]
  stateDataFiltered <- stateDataFiltered[minIdx:maxIdx,]
  ...
  single_state_results <- list(data = stateDataFiltered, 
                               ...
                               sims = R0,
                               location = state)
  save("single_state_results", "stateDataObserved", "opt", 
       "epidemicStartDate", "epidemicEndDate",
       file = resultsFileName, compress = "bzip2")
} 
