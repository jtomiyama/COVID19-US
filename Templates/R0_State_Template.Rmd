...
save(eps_df, paste0(TEMPLATE_STATE_IDX, "_model_eps.rda"))
...
obs_data <- analysisResults[[1]]$single_state_results$data %>%
  select(date) %>%
  full_join(analysisResults[[1]]$stateDataObserved) %>%
...
save(outFilt, file = paste0(TEMPLATE_STATE_IDX, "_R0.rda"))
```
...
save(outFilt, file = paste0(TEMPLATE_STATE_IDX, "_CumMort.rda"))
...
kable(out.ToPrint, digits = 1,align = c("c"), caption = "Incidence: Latest") %>% 
  kable_styling(bootstrap_option = c("striped", "condensed"), 
                full_width = FALSE) 
save(OutFilt, paste0(TEMPLATE_STATE_IDX, "_CumCases.rda"))
