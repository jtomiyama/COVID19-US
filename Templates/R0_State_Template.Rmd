...
rsltFileIndicator <- grepl(clean_idx, substr(dir(resultsDirectoryName), 1, 2)) &
                     grepl(paste0("[",paste0(TEMPLATE_INTERVENTION_TYPES,collapse = ""),"]"),
...
Model 1 assumes an instantaneous shift in (a function of) the probability of exposure once the public policy order starts and again when it stops. Model 2 assumes a piecewise linear effect with time. Model 3 specifies a basis spline expansion over the mandate time. For states without a stay at home order, a basis spline expansion on the observed epidemic time are used. See the results section for visualizations of these effects.
...
Intervention Types Used: `cat(intervention_types)`
...
cat("\n\nBelow are the terminating epsilons for each intervention model. For reference, if a model whose mortality predictions were off by a count of 5 at every time point the terminating epsilon would be:",
    sqrt(5^2*nrow(analysisResults[[1]]$single_state_results$data))
    )
...
obs_I <- analysisResults[[1]]$stateDataObserved$casesNonCum %>%
  filter(date <= max(dtobj))
obs_R <- analysisResults[[1]]$stateDataObserved$deathsNonCum %>%
  filter(date <= max(dtobj))
for(i in 1:length(intervention_types)){
  cat("### Intervention Type", intervention_types[i],"\n")
    mod <- analysisResults[[i]]$single_state_results$result$modelObject
  ...
  pred_deaths <- Reduce(cbind, lapply(sims, function(x){x$R_star}))
  pq <- c(0.1, 0.25, 0.5,0.75,0.9)
  qdeaths <- apply(pred_deaths, 1, quantile, probs = pq)
...
rwd <- paste0(resultDirectory, "2020-05-02/")
if(dir.exists(rwd)){
  rsltFileIndicator <- grepl(clean_idx, substr(dir(rwd), 1, 2)) &
                     grepl(paste0("[",
                           paste0(TEMPLATE_INTERVENTION_TYPES, collapse = ""),
                                  "]"),
                           substr(dir(rwd), 4, 4))
if(sum(rsltFileIndicator) > 0){
  resultFiles <- paste0(rwd,"/", 
                        dir(rwd)[rsltFileIndicator])
}else{
  stop("\nResults files were not found for", TEMPLATE_STATE_IDX, "\n")
}
analysisResults <- lapply(resultFiles, function(rf){
...
suppressWarnings({
  ...
  cat("Reference Results Doesn't Exist")
}
